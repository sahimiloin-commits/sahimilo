...
async function searchProfessionals(){
  ...
  const wanted=selectedServices.map(service=>service.toLowerCase());data=data.filter(p=>(p.services||[]).some(service=>wanted.includes(String(service).toLowerCase())));const priority={available_today:0,available_tomorrow:1,currently_busy:2,temporarily_unavailable:3};data=data.filter(p=>p.availability_status!=="temporarily_unavailable").sort((a,b)=>(priority[a.availability_status]??3)-(priority[b.availability_status]??3)||Number(b.rating||0)-Number(a.rating||0));
