...
async function searchProfessionals(){
  ...
  let query=db.from("professionals").select("id,name,phone,whatsapp,services,service_villages,service_cities,service_districts,service_pincodes,experience_years,bio,photo_url,rating,reviews_count,status,verified_mobile,verified_whatsapp,verified_by_admin").eq("status","approved").eq("verified_by_admin",true).order("rating",{ascending:false}).limit(100);if(service)query=query.contains("services",[service]);
  ...
  const badges=p=>[p.verified_by_admin?'<span class="public-trust-badge verified">✓ SahiMilo Verified</span>':'',p.verified_mobile?'<span class="public-trust-badge">✓ Mobile</span>':'',p.verified_whatsapp?'<span class="public-trust-badge">✓ WhatsApp</span>':''].filter(Boolean).join("");
