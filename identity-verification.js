...
async function load(){
 ...
 record=data;render();const {data:ownerProfile}=await db.from("user_profiles").select("account_claim_status").eq("id",user.id).maybeSingle();const claimed=ownerProfile?.account_claim_status==="claimed",customerBadge=$("customerClaimStatus"),professionalBadge=$("accountClaimBadge");if(customerBadge){customerBadge.textContent=claimed?"Account claimed":"Account access pending";customerBadge.className="status-badge "+(claimed?"status-approved":"status-pending")}if(professionalBadge){professionalBadge.textContent=(claimed?"✓ ":"○ ")+(claimed?"Account claimed":"Account access pending");professionalBadge.className="trust-badge "+(claimed?"verified":"pending");await initAvailability()}
}

async function initAvailability(){
 if(!$("accountClaimBadge"))return;
 const {data:professional}=await db.from("professionals").select("availability_status,availability_updated_at").eq("user_id",user.id).maybeSingle();
 const anchor=$("assignedHistory")?.closest(".portal-panel");if(!anchor)return;
 const section=document.createElement("section");section.className="portal-panel";section.innerHTML='<div class="history-top"><div><h2>My availability</h2><p class="account-note">Customer ko batayein ki aap kab kaam ke liye available hain.</p></div><span id="availabilitySavedStatus" class="status-badge"></span></div><form id="availabilityForm"><label class="field"><span>Availability status *</span><select id="professionalAvailability"><option value="available_today">Available today</option><option value="available_tomorrow">Available tomorrow</option><option value="currently_busy">Currently busy</option><option value="temporarily_unavailable">Temporarily unavailable</option></select></label><button id="saveAvailability" class="primary-btn" type="submit">Save Availability</button><p id="availabilityMessage" class="notice hidden" role="alert"></p></form>';
 anchor.parentNode.insertBefore(section,anchor);$("professionalAvailability").value=professional?.availability_status||"temporarily_unavailable";
 const availabilityLabels={available_today:"Available today",available_tomorrow:"Available tomorrow",currently_busy:"Currently busy",temporarily_unavailable:"Temporarily unavailable"};$("availabilitySavedStatus").textContent=availabilityLabels[$("professionalAvailability").value];
 $("availabilityForm").onsubmit=async event=>{event.preventDefault();const status=$("professionalAvailability").value,button=$("saveAvailability"),notice=$("availabilityMessage");button.disabled=true;const {error:updateError}=await db.from("professionals").update({availability_status:status,availability_updated_at:new Date().toISOString()}).eq("user_id",user.id);button.disabled=false;if(updateError){notice.textContent="Availability save nahi hui: "+updateError.message;notice.className="notice notice-error";return}$("availabilitySavedStatus").textContent=availabilityLabels[status];notice.textContent="Availability customers ke liye update ho gayi.";notice.className="notice notice-ok"};
}
